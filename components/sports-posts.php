<?php
	$latest_sports_posts = new WP_Query([
		'post_type'      => 'post', // Only posts
		'posts_per_page' => 4,      // 9 posts
		'category_name'  => 'deportes', // Only 'Política' category
	]);
	...
	if ($latest_sports_posts->have_posts()) {
		while ($latest_sports_posts->have_posts()) {
			$latest_sports_posts->the_post();
...
<div class="space-y-12 basis-1/2">
	...
	<div class="w-full flex flex-col gap-y-3 gap-x-15 transition-all transition-duration-150 md:flex-row lg:flex-col">
		<div class="basis-1/2">
