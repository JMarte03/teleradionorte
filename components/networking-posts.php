<?php
	$latest_networking_posts = new WP_Query([
		'post_type'      => 'post', // Only posts
		'posts_per_page' => 4,      // 
		'category_name'  => 'intercambios', // 
	]);

	$posts_array = [];
	$default_image = get_template_directory_uri() . '/resources/img/trn-placeholder.png';

	if ($latest_networking_posts->have_posts()) {
		while ($latest_networking_posts->have_posts()) {
			$latest_networking_posts->the_post();
