	<footer id="colophon" class="relative bottom-0 bg-white w-full mt-20" role="contentinfo">
		...
		<section class="p-12 flex flex-col gap-y-7 lg:flex-row lg:justify-between lg:items-center bg-brandGreen">
			<div class="space-y-2 lg:w-[50%]">
				<h3 class="text-white text-2xl font-gothicExpanded">Suscríbete a nuestro boletín</h3>
				<p class="font-encodeSans text-md text-white">Mantente informado con las últimas noticias, anuncios y artículos.</p>
			</div>
			<form class="w-full space-y-2 lg:w-auto">
				<input placeholder="Correo electrónico" type="email" class="input w-full p-6 bg-white text-gray-500 font-encodeSans">
				<button type="submit" class="btn w-full border-0 p-6 shadow-none bg-darkGreen text-white font-encodeSans transition-all transition-duration-150 hover:bg-darkGreen/80">Suscribirse</button>
			</form>
			</div>
		</section>
		<!-- Footer -->
		<section class="py-8 px-12 flex flex-col gap-y-5 bg-darkGreen md:flex-row md:justify-between md:items-center">
			<p class="text-white font-encodeSans font-light">&copy; <?php echo date_i18n( 'Y' );?> - Teleradionorte. Todos los derechos reservados.</p>
